...
def train_val(train_val_data, model, mode, bs, epochs, criterion, optimizer, early_stopper, rand_samplers, logger, model_dim, n_hop=2):
  ...
  for epoch in range(epochs):
    ...
    for k in tqdm(range(num_batch)):
      ...
      ## The emb of the classical model and the Logits of the classical model emb are obtained by using the two newly defined functions in module
      with torch.no_grad():
              dedg_src_emb, dedg_tgt_emb, dedg_bad_emb = model.get_dedg_emb(src_l_cut, tgt_l_cut, bad_l_cut, ts_l_cut, e_l_cut)
              pos_prob_other, neg_prob_other, other_src_emb, other_tgt_emb, other_bad_emb = model.get_other_model_score(src_l_cut, tgt_l_cut, bad_l_cut, ts_l_cut, e_l_cut)
      ...
      criterion2 = torch.nn.L1Loss() #### use BCE
      criterion3 = torch.nn.MSELoss()
      loss = (criterion(pos_prob, pos_label) + criterion(neg_prob, neg_label)) + 0.1*(criterion3(dedg_src_emb, other_src_emb) + criterion3(dedg_tgt_emb, other_tgt_emb)) + 0.05*(criterion2(pos_prob, pos_prob_other))
      loss.backward()
      optimizer.step()
    ...
    dedg_results(logger, train_time, "train_time")
