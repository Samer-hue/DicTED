...
class DEDG(torch.nn.Module):
  def __init__(self, n_feat, n_feat_other, e_feat, memory_dim, total_nodes, get_checkpoint_path=None, get_ngh_store_path=None, get_self_rep_path=None, get_prev_raw_path=None, time_dim=2, pos_dim=0, n_head=4, num_neighbors=['1', '32'],
      dropout=0.1, linear_out=False, verbosity=1, seed=1, n_hops=2, replace_prob=0.9, self_dim=100, ngh_dim=8, device=None):
    super(DEDG, self).__init__()
    self.logger = logging.getLogger(__name__)
    self.dropout = dropout
    self.n_feat_th = torch.nn.Parameter(torch.from_numpy(n_feat.astype(np.float32)), requires_grad=False)
    ## Added parameters from the classical model emb
    self.n_feat_other_th = torch.nn.Parameter(torch.from_numpy(n_feat_other.astype(np.float32)), requires_grad=False)
    ...
    ## Correspondingly, add a embedding layer and encoder
    self.node_other_embed = torch.nn.Embedding.from_pretrained(self.n_feat_other_th, padding_idx=0, freeze=False)
  ...
  def position_bits(self, bs, hop):
    ...
    ## 功能不变，因为<<在新版本被弃用
    return (torch.ones(bs * self.num_neighbors[hop], device=self.device) * 2**hop).to(torch.int)
  ...
  def contrast(self, src_l_cut, tgt_l_cut, bad_l_cut, cut_time_l, e_idx_l=None, test=False):
    ...
    ## 功能不变，因为<<在新版本被弃用
    tgt_pos_raw = (pos_raw[src_nghs:src_nghs + tgt_nghs] * 2**3).to(torch.int)
    bad_pos_raw = (pos_raw[src_nghs + tgt_nghs:] * 2**3).to(torch.int)
  ...
  ### Define a function to obtain the emb obtained after various operations on the model
  def get_dedg_emb(self, src_l_cut, tgt_l_cut, bad_l_cut, cut_time_l, e_idx_l=None, test=False):
    start = time.time()
    start_t = time.time()
    batch_size = len(src_l_cut)
    ...
    tgt_self_rep = self.updated_self_rep(tgt_th)
    bad_self_rep = self.updated_self_rep(bad_th)
    return src_self_rep, tgt_self_rep, bad_self_rep
  
  ## Define functions that compute logits using EMBs from classical models
  def get_other_model_score(self, src_l_cut, tgt_l_cut, bad_l_cut, cut_time_l, e_idx_l=None, test=False):
    start = time.time()
    start_t = time.time()
    batch_size = len(src_l_cut)
    ...
    src_self_rep = self.node_other_embed(src_th)
    tgt_self_rep = self.node_other_embed(tgt_th)
    bad_self_rep = self.node_other_embed(bad_th)
    p_score, n_score, attn_score = self.forward(ngh_and_batch_id_p, ngh_and_batch_id_n, features, batch_size, src_self_rep, tgt_self_rep, bad_self_rep)
    return p_score.sigmoid(), n_score.sigmoid(),src_self_rep, tgt_self_rep, bad_self_rep
  ...
  def forward(self, ngh_and_batch_id_p, ngh_and_batch_id_n, feat, bs, src_self_rep=None, tgt_self_rep=None, bad_self_rep=None):
    ...
    #print("src_self_rep.shape:", src_self_rep.shape)
    #print("p_embed.shape:",p_embed.shape)
