...
from my_emb_to_npy import emb_to_npy
from n2v_main import n2v
import os
...
## Two parameters are added. weight1 indicates the weight of the new emb when combined with the original emb
WEIGHT1 = args.weight1
## model Indicates the classic model used
#MODEL = args.model
...
## If a model is used, a different initial emb is used

if DATA == 'socialevolve_1month':
    emb_len = 70
elif DATA == 'socialevolve':
    emb_len = 74   
else:
    emb_len = 172
    
if os.path.exists("./processed/{}/ml_{}_node_gae.npy".format(DATA,DATA)):
    n_feat2_gae = np.load("./processed/{}/ml_{}_node_gae.npy".format(DATA,DATA))
else:
    emb_to_npy(DATA, 'gae', size=emb_len)
    n_feat2_gae = np.load("./processed/{}/ml_{}_node_gae.npy".format(DATA,DATA))
print(n_feat2_gae.shape)
if os.path.exists("./processed/{}/ml_{}_node_TGN.npy".format(DATA,DATA)):
    n_feat2_tgn = np.load("./processed/{}/ml_{}_node_TGN.npy".format(DATA,DATA))
else:
    from TGN.main import main_TGN
    main_TGN(DATA, emb_len)
    emb_to_npy(DATA, 'TGN', emb_len)
    n_feat2_tgn = np.load("./processed/{}/ml_{}_node_TGN.npy".format(DATA,DATA))
print(n_feat2_tgn.shape)
if os.path.exists("./processed/{}/ml_{}_node_SDCN.npy".format(DATA,DATA)):
    n_feat2_sdcn = np.load("./processed/{}/ml_{}_node_SDCN.npy".format(DATA,DATA))
else:
    from SDCN.sdcn import SDCN_main
    SDCN_main(DATA, emb_len, emb_len)
    emb_to_npy(DATA, 'SDCN', emb_len)
    n_feat2_sdcn = np.load("./processed/{}/ml_{}_node_SDCN.npy".format(DATA,DATA))
print(n_feat2_sdcn.shape)

if os.path.exists("./processed/{}/ml_{}_node_TREND.npy".format(DATA,DATA)):
    n_feat2_trend = np.load("./processed/{}/ml_{}_node_TREND.npy".format(DATA,DATA))
else:
    from TREND.main import TREND_main
    TREND_main(DATA, emb_len)
    emb_to_npy(DATA, 'TREND', emb_len)
    n_feat2_trend = np.load("./processed/{}/ml_{}_node_TREND.npy".format(DATA,DATA))
print(n_feat2_trend.shape)

n_feat2 = a1 * n_feat2_gae + a2 * n_feat2_sdcn + a3 * n_feat2_tgn + (1-a1-a2-a3)*n_feat2_trend
print(n_feat2.shape)
if DATA == 'wikipedia' or DATA == 'reddit':
    n_feat1 = np.load('./processed/{}/ml_{}_node.npy'.format(DATA,DATA))
    n_feat = (1-WEIGHT1)*n_feat1 + WEIGHT1*n_feat2
else:
    n_feat=n_feat2
...
for run in range(args.run):
  ...
  ## Add one parameter, input the emb obtained from the classical model, and calculate the use of loss
  dedg = DEDG(n_feat, n_feat2, e_feat, memory_dim, max_idx + 1, time_dim=TIME_DIM, pos_dim=POS_DIM, n_head=ATTN_NUM_HEADS, num_neighbors=num_neighbors, dropout=DROP_OUT,
  ...
  optimizer = torch.optim.Adam(dedg.parameters(), lr=LEARNING_RATE)
  criterion = torch.nn.BCELoss()
  early_stopper = EarlyStopMonitor(tolerance=TOLERANCE)

  # start train and val phases
  train_val(train_val_data, dedg, args.mode, BATCH_SIZE, NUM_EPOCH, criterion, optimizer, early_stopper, rand_samplers, logger, model_dim, n_hop=NUM_HOP)
  ...
  if args.mode == 'i':
    dedg.reset_store()
    dedg.reset_self_rep()
    train_acc, train_ap, train_f1, train_auc = eval_one_epoch('test for {} nodes'.format(args.mode), dedg, all_train_val_rand_sampler, all_train_val_src_l, all_train_val_tgt_l, all_train_val_ts_l, all_train_val_label_l, all_train_val_e_idx_l, bs=32)
  test_start = time.time()
  test_acc, test_ap, test_f1, test_auc = eval_one_epoch('test for {} nodes'.format(args.mode), dedg, test_rand_sampler, test_src_l, test_tgt_l, test_ts_l, test_label_l, test_e_idx_l)
  test_end = time.time()
  ...
  # save model
  logger.info('Saving DEDG model ...')
  torch.save(dedg.state_dict(), best_model_path)
  logger.info('DEDG model saved')
  ...
  total_end = time.time()
  print("DEDG experiment statistics:")
  if args.mode == "t":
    dedg_results(logger, transductive_auc, "transductive_auc")
    dedg_results(logger, transductive_ap, "transductive_ap")
  else:
    dedg_results(logger, inductive_auc, "inductive_auc")
    dedg_results(logger, inductive_ap, "inductive_ap")
  
  dedg_results(logger, test_times, "test_times")
  dedg_results(logger, early_stoppers, "early_stoppers")
  total_time.append(total_end - total_start)
  dedg_results(logger, total_time, "total_time")